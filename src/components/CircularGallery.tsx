...
// Adapted from ReactBits "Circular Gallery" by David Haz (MIT License)
...
function autoBind(instance: object): void {
  ...
  Object.getOwnPropertyNames(proto).forEach(key => {
    if (key !== "constructor" && typeof proto[key] === "function") {
      (instance as Record<string, unknown>)[key] = (proto[key] as (...args: unknown[]) => unknown).bind(instance);
    }
  });
}
...
class Title {
  ...
  createMesh() {
    ...
    const textHeightScaled = this.plane.scale.y * 0.06;
...
class App {
  ...
  onMouseMove(e: MouseEvent) {
    ...
    this.mouse = {
      x: ((e.clientX - rect.left) / rect.width - 0.5) * this.viewport.width,
      y: ((e.clientY - rect.top) / rect.height - 0.5) * this.viewport.height
    };
  }
